...
def main():
    ...
    for i in range(args.number):
        ...
        if i % max(5, args.number / 50) == 0:
            progress(i, args.number,       status='{:,d} @{:,.2f} rps. Bytes: {:,d} <{:,.2f} bytes>'.format(i, float(i) / (time_now - time_start), bytecount, float(bytecount)/float(i+1)) )
    ...
    progress(args.number, args.number, status='{:,d} @{:,.2f} rps. Bytes: {:,d} <{:,.2f} bytes>'.format(args.number, float(args.number) / (time_now - time_start), bytecount, float(bytecount)/float(args.number)) )
